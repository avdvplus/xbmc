...
void KODI::VIDEO::GUILIB::OpenDialogSelectVideoStream()
{
  ...
  const auto selectedItem = OpenSelectDialog(*dialog, 291, itemsToDisplay);
  if (selectedItem)
  {
...
void KODI::VIDEO::GUILIB::OpenDialogSelectAudioStream()
{
  ...
  const auto selectedItem = OpenSelectDialog(*dialog, 292, itemsToDisplay);
  if (selectedItem)
  {
    const int id = selectedItem->GetProperty("stream.id").asInteger32(STREAM_ID_NONE);
...
void KODI::VIDEO::GUILIB::OpenDialogSelectSubtitleStream()
{
  ...
  const auto selectedItem = OpenSelectDialog(*dialog, 287, itemsToDisplay);
  if (selectedItem)
  {
    const int id = selectedItem->GetProperty("stream.id").asInteger32(STREAM_ID_NONE);
